			...
			foreach($result->binding as $binding)
			{
				...
				if($binding->literal)
				{
					...
					$row[$name . '_type'] = 'literal';
                                        $tmp = $binding->literal->attributes();
                                        $row[$name.'_datatype'] = (string)$tmp[0];
                                        $tmp = $binding->literal->attributes('http://www.w3.org/XML/1998/namespace');
                                        if (is_null($tmp['lang']) === false) {
                                           $row[$name.'_lang'] = (string) $tmp['lang'];
                                        }                                        
